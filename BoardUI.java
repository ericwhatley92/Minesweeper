...
public class BoardUI extends JPanel implements MouseListener, MouseMotionListener{
	...
	public void mouseClicked(MouseEvent click) {
		...
		//if a non-mined area is clicked the recursive method is called
    ...
    public void mouseReleased (MouseEvent e) {
		/*int a = e.getButton();
		SingleButton source = (SingleButton)e.getSource();
		ZeroButton(source.getRow(), source.getColumn());
			if(checkGameover(source, a)) {
				t.stop();
				gameOver();
...
/**********************************PRIVATE FUNCTIONS**********************************/
	...
	private int Number(int row, int col) {
		...
		//Sets colors of numbers and numbers
		switch(bombs) {
			case 0:
				array[row][col].setForeground(Color.BLACK);
				array[row][col].setBackground(Color.GRAY);
